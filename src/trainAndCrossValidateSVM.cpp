...
int main(int argc, char** argv)
{
  ...
  string cascadeClassifierName;
	...
	if(perSubject)
	{
		...
		while(params.min_val <= params.max_val)
		{
			...
			for(int i = 0; i < numSubjects; i++)
			{
				...
				CvSVM svm(trainData, trainResponses, Mat(), Mat(), svmParams);
				Mat responses;
				learningAlgorithmPredict(&svm, validationData, responses, 
			...
			cout << "Validation error is " << validationError*100
				<< "\% for C=" << svmParams.C << endl;
			if(validationError < bestValidationError)
			{
				bestValidationError = validationError;
				bestSVMParams = svmParams;
			}
			params.min_val *= params.step;
		}
	}
	else
	{
	...
	cout << "Best C=" << bestSVMParams.C << " gives validation error "
	...
	if(!saveClassifierLocation.empty())
	{
		...
		CvSVM svm(trainData, trainResponses, Mat(), Mat(), bestSVMParams);
